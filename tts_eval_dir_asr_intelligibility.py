...
def main():
    ...
    metric = te.ASRIntelligibility(device)

    if args.rerun or not Path(args.out_filepath).exists():
        results = metric(args.pred_dir, args.gt_transcript_file)
    ...
    print(f"ASRIntelligibility (CER): {cer_result}")
    print(f"ASRIntelligibility (WER): {wer_result}")
